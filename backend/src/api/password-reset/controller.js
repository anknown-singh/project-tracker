...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your project-tracker account.<br>
        ...
        &mdash; project-tracker Team
      `
      return sendMail({ toEmail: email, subject: 'project-tracker - Password Reset', content })
    })
